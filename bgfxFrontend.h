...
void bgfx_stb_font_cache::pregenGlyphs_pack (SSF_VECTOR <tempGlyph> & tempGlyphs, SSF_VECTOR <stbrp_rect> & rects, bool force)
		...
		for (unsigned int i = 0; i < rects.size(); ++i) {
			...
			if (r.was_packed) {
				//Logger_ldbg("SDL_STB pregenGlyphs_pack {} {}", t.target, char(t.target & 0xff));
...
void bgfx_stb_font_cache::genGlyph_writeData2 (uint32_t const codepoint, sttfont_glyph * gOut, unsigned char * bitmap2, int w, int h, bool firstCall)
		...
		if (r.was_packed) {
			...
			//Logger_ldbg("SDL_STB genGlyph_writeData2 {} {} {}, atlas: {} {} {} {}, mem: {}", codepoint, r.id, char(codepoint), r.x, r.y, r.w, r.h, Utils::binToHex(SSF_STRING((const char*) bitmap2, bx::min(w*h*4, 64))));			
			#warning Known issue: vk backend will sometimes not copy pixels to texture
			// Possible fix - discard current texture and upload?
			// putting in a loop (for i = 0, 100) fixes vk issue at cost of perfomance
			...
			//Logger_ldbg("SDL_STB genGlyph_writeData2 REJECTED CODEPOINT {} {}", codepoint, char(codepoint));
			activeAtlas->isFull = true;
			}
