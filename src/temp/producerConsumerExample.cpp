...
public:
	...
	void producerExample() {
		...
		while (true) {
			mCommandBuffer.push_back(command(COMMAND_TYPE_RECT, -1, 0, 0, 50, 50));
			mCommandBuffer.push_back(command(COMMAND_TYPE_PRETEXT, prt.handle, 10, 10));
			pcfc_handle textHandle = mPcCache.pushText(5,5, "Direct Text from Producer Thread");
	...
	void consumerExample() {
		...
		while (true) {
			...
			mPcCahce.dispatchPrerenders<sdl_stb_prerendered_text>(); // must be called to actually prerender the text
			...
			for (const command & mCommand : mCommandBuffer.mCommandsConsumer) {
				switch (mCommand.type) {
					case COMMAND_TYPE_PRETEXT: 
						mPcCache.dispatchPrerendered(mCommand.x, mCommand.y, mCommand.xs);
						continue;
					case COMMAND_TYPE_TEXT
						mPcCache.dispatchText(mCommand.x);
						continue;
					case COMMAND_TYPE_RECT
						{
						SDL_Rect
						drawRectangle(mCommand.x, mCommand.y, mCommand.w, mCommand.h);
						continue;
						}
					default:
						continue;
...
int main(int argc, char**argv) {		
	...
	{
	std::string fontData;
	readFileRaw("sdlStbFont/fonts/NotoSans-Regular.ttf", fontData);
	ex.init(fontData);
	}
